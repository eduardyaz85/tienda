...
class Sistema {
    ...
    public function getTruncateTabla($tabla) {
        $rs = ($this->_db->query("TRUNCATE TABLE $tabla"));
        if ($rs) {
            DwAudit::info("Se ha realizado el vaciado de la $tabla");
        } else {
            DwAudit::error("Se ha generado un error al borrar la tabla $tabla");
        }
        return $rs;
    }
