...
class Perfil extends ActiveRecord {
    ...
    /**
     * Constante para definir el id del Administrador
     */
    const ADMIN = 2;
    
    /**
     * Constante par definir el id de SUPERVISOR
     */
    const SUPERVISOR = 3;

    /**
     * Constante par definir el id del cliente distribuidor
     */
    const DISTRIBUIDOR = 4;
    
    /**
     * Constante par definir el id del cliente
     */
    const CLIENTE = 5;
    ...
    public function getListadoPerfil($estado='todos', $order='', $page=0) {                   
        $columns = 'perfil.*, COUNT(usuarios.id) AS usuarios';        
        $join = 'LEFT JOIN usuarios ON perfil.id = usuarios.perfil_id ';
