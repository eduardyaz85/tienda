...
class Backup extends ActiveRecord {
    ...
    public function getAjaxBackup($field, $value, $order='', $page=0) {
        ...
        $join       = 'INNER JOIN usuario ON usuarios.id = backup.usuarios_id ';
    ...
    public static function restoreBackup($id, $path='') {
        ...
        if(!$result) {
            ...
            foreach($old_backup as $backup) {
                if($backup->id >= $obj->id ) {
                    $obj->sql("REPLACE INTO `backup` (`id`,`usuarios_id`,`denominacion`,`tamano`,`archivo`,`backup_at`) VALUES ('$backup->id', '$backup->usuarios_id', '$backup->denominacion', '$backup->tamano', '$backup->archivo', '$backup->backup_at')");
                }
            }
            if($obj) {
