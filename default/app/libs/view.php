...
class View extends KumbiaView {
    ...
    public static function process($modulo, $proceso = null, $title = true) {
        return self::partial('process', false, array('modulo' => $modulo, 'proceso' => $proceso, 'titulo' => $title));
    }
    ...
    public static function exception(KumbiaException $e) {
        if (PRODUCTION) {
            ...
            Flash::warning('Oops! hemos realizado algún procedimiento mal... <br />Inténtalo nuevamente!');
        } else {
            Flash::error($e->getMessage());
            Flash::error("Detalle del error: " . $e->getTraceAsString());
        }
        if (Session::has('exception_counter')) {
            if (Session::get('exception_counter') > 2) {
                Flash::info('Si el problema persiste contacta con el administrador del sistema.');
    ...
    public static function report($formato) {
        ...
        $template = ($formato == 'html') ? 'imprimir' : NULL;
        if ($formato == 'error') {
            self::error();
            ...
            Flash::error('Error: ACCESO DENEGADO. El formato del reporte es incorrecto.');
            self::error();
        } else {
