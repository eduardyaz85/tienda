...
class MiCuentaController extends BackendController {
    ...
    public function index() {
        ...
        if(Input::hasPost('usuario')) {
            $usuario = Usuarios::setUsuario('update', Input::post('usuario'), array('repassword'=>Input::post('repassword'), 'oldpassword'=>Input::post('oldpassword'), 'id'=>$usuario->id, 'login'=>$usuario->login, 'perfil_id'=>$usuario->perfil_id));
            if($usuario) {                
                ...
                if($app_ajax_old != $usuario->app_ajax) {
                    ...
                    return Redirect::to('/sistema/mi_cuenta');
                }
