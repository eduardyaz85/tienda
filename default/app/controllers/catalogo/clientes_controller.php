...
class ClientesController extends BackendController {
    ...
    public function listar($order = 'order.razon_social.asc', $page = 'pag.1') {
        ...
        $this->empresas = (new Empresas)->getListaEmpresas($tipo = Empresas::CLIENTE, 'todos', $order, $page);
    ...
    public function buscar($field = 'razon_social', $value = 'none', $order = 'order.id.asc', $page = 1) {
        ...
        if (empty(Input::post('value'))) {
            Flash::info('Ingrese un empresa');
            return Redirect::toAction('listar');
        }

        $empresas = new Empresas();
        $empresa = $empresas->getBuscaEmpresas($field, $value, $order, $page, $tipo = Empresas::CLIENTE);
    ...
    public function agregar() {
        if (Input::hasPost('empresa') && Input::hasPost('sucursal')) {
            if (Empresas::setEmpresas('create', Input::post('empresa'), array('type' => Empresas::CLIENTE))) {
                Flash::valid('El empresa se ha creado correctamente.');
                return Redirect::toAction('listar');
            }
        }
    ...
    /**
     * Método para agregar contactos del empresa
     */
    public function contacto($key) {
        ...
        $empresas = new Empresas();
        if (!$empresas->getInformacionEmpresas($id)) {
            Flash::error('No se pudo verificar la informacion del empresa');
            return Redirect::toAction('listar');
        }
        if (Input::hasPost('contacto')) {
            if (Contactos::setContacto('create', Input::post('contacto'), array('empresas_id' => $empresas->id, 'estado' => Contactos::ACTIVO))) {
                $key = Security::setKey($empresas->id, 'shw_empresa');
    ...
    public function editar($key) {
        if (!$id = Security::getKey($key, 'upd_empresa', 'int')) {
            return Redirect::toAction('listar');
        }
        ...
        if (Input::hasPost('empresa') && Input::hasPost('sucursal')) {
            if (Empresas::setEmpresas('update', Input::post('empresa'), array('id' => $empresas->id))) {
                Flash::valid('El cliente se ha actualizado correctamente!');
                return Redirect::toAction('listar');
            }
        }

        $this->sucursal = (new Establecimientos())->getInformacionEstablecimiento($empresas->id);
    ...
    public function modifica($key) {
        ...
        if (Input::hasPost('contacto')) {
            if (Contactos::setContacto('update', Input::post('contacto'), array('id' => $id))) {
                $key = Security::setKey($contactos->empresas_id, 'shw_empresa');
    ...
    public function estadocontacto($tipo, $key) {
        ...
        if (!$contacto->find_first($id)) {
            ...
            $key = Security::setKey($contacto->empresas_id, 'shw_empresa');
            if ($contacto->id <= 0) {
            ...
            if ($tipo == 'inactivar' && $contacto->estado == Contactos::INACTIVO) {
                ...
                Flash::info('El contacto ya se encuentra activo');
            } else {
    ...
    public function ver($key) {
        ...
        $this->contactos = (new Contactos)->getInformacionContactosPorTipo('todos', 'order.nombre.desc', $empresas->id, $empresas->type);
